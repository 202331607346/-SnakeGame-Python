...
class SnakeGame:
    ...
    def create_info_panel(self):
        # 玩家信息 - 加大字体，调整间距
        tk.Label(self.info_frame, text=f"贪吃蛇游戏",
                 font=("Arial", 18, "bold"), fg='#f1c40f', bg='#2c3e50').pack(pady=10)
        tk.Label(self.info_frame, text=f"玩家: {self.username}",
                 font=("Arial", 15, "bold"), fg='white', bg='#2c3e50').pack(pady=5)

        # 分隔线
        tk.Frame(self.info_frame, height=3, bg='#f1c40f').pack(fill=tk.X, padx=25, pady=10)
        ...
        self.add_score_label = tk.Label(self.info_frame, text="单食得分: 10",
                                        font=("Arial", 12), fg='#3498db', bg='#2c3e50')
        self.add_score_label.pack(pady=3)

        self.speed_label = tk.Label(self.info_frame, text="速度: 5/10",
                                    font=("Arial", 12), fg='#e74c3c', bg='#2c3e50')
        self.speed_label.pack(pady=3)

        self.status_label = tk.Label(self.info_frame, text="✅ 游戏运行中",
                                     font=("Arial", 12, "bold"), fg='#2ecc71', bg='#2c3e50')
        self.status_label.pack(pady=10)

        # 操作说明标题
        tk.Label(self.info_frame, text="🎮 快捷键操作", font=("Arial", 14, "bold"),
        ...
        for key, func in instructions:
            ...
            tk.Label(frame, text=key, font=('Arial', 10, 'bold'), fg='#f1c40f', bg='#2c3e50', width=10, anchor='w').pack(side=tk.LEFT)
            tk.Label(frame, text=func, font=('Arial', 10), fg='white', bg='#2c3e50', anchor='w').pack(side=tk.LEFT)

        tk.Label(self.info_frame, text="💡 点击游戏区域生效按键",
                 font=("Arial", 10, "bold"), fg='#2ecc71', bg='#2c3e50').pack(pady=15)

    def on_up(self, event):
        if self.game_running and not self.game_paused:
            if self.direction != DOWN and self.next_direction != DOWN:
                self.next_direction = UP
                return "break"

    def on_down(self, event):
        if self.game_running and not self.game_paused:
            if self.direction != UP and self.next_direction != UP:
                self.next_direction = DOWN
                return "break"

    def on_left(self, event):
        if self.game_running and not self.game_paused:
            if self.direction != RIGHT and self.next_direction != RIGHT:
                self.next_direction = LEFT
                return "break"

    def on_right(self, event):
        if self.game_running and not self.game_paused:
            if self.direction != LEFT and self.next_direction != LEFT:
                self.next_direction = RIGHT
                return "break"
    ...
    def update_score_display(self):
        ...
        self.add_score_label.config(text=f"单食得分: {self.add_score}")
        self.update_speed_display()
    ...
    def check_collision(self):
        ...
        if (head[0] < 0 or head[0] >= GAME_WIDTH or
            ...
            self.status_label.config(text="❌ 撞墙了", fg='#e74c3c')
            return True

        if head in self.snake[1:]:
            self.status_label.config(text="❌ 咬到自己了", fg='#e74c3c')
            return True
    ...
    def toggle_pause(self):
        if self.game_running:
            ...
            if self.game_paused:
                self.status_label.config(text="⏸ 游戏已暂停", fg='#f39c12')
            else:
                self.status_label.config(text="✅ 游戏运行中", fg='#2ecc71')
                self.game_loop()
